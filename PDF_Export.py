...
def build_legend(S):
    ...
    for bg, accent, label, sub in items:
        cells.append(
            Table(
                [
                    [Paragraph(label, S["legend_label"])],
...
def build_timetable_table(rows, S):
    ...
    for i, row in enumerate(rows):
        ...
        fill = bg if i % 2 == 0 else colors.Color(bg.red * 0.97, bg.green * 0.97, bg.blue * 0.97)
        ...
        row_styles += [
            ...
            ("LEFTPADDING", (0, row_idx), (0, row_idx), 6),  # Date Indent
        ]
...
# send_daily_nudge(rows, recipient_email=os.getenv("Receiver_GMAIL_ID"), pdf_path="My_Timetable.pdf")
